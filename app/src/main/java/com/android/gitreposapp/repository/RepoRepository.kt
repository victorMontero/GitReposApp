...
class RepoRepository(val db: RepositoryDatabase) {
    ...
    suspend fun insert(repository : RepositoryResponseItem) = db.getRepositoryDao().insert(repository)

    fun getSavedRepository() = db.getRepositoryDao().getAllRepositories()

    suspend fun deleteRepository(repository : RepositoryResponseItem) = db.getRepositoryDao().deleteRepository(repository)
}